...
public:
	...
	Octree(int amount) : mRoot(glm::vec3(0.f), 10.f), mAmount(amount)
	{
		mTheta = 0.5f;
		mActive = true;
		...
		for (int i = 0; i < mAmount; i++)
		{
			...
			vel = glm::normalize(vel) * orbital_vel / 220000.f;
			mData.emplace_back(pos, vel, 1.f);
		}
		mRoot.build(1.f, &mRoot);
		for (auto& body : mData)
		{
			mRoot.populate(&body);
		}
	...
	const Body* getBodiesData() const noexcept
	{
		return mData.data();
	}

	const int getBodiesAmount() const noexcept
	{
		return mAmount;
	}
